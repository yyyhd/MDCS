...
def fit_one_epoch(model_train, model, focal_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        if iteration >= epoch_step:
            break
        images1,images2, targets = batch[0],batch[1], batch[2]
        with torch.no_grad():
            if cuda:
                images1  = images1.cuda(local_rank)
                images2 = images1.cuda(local_rank)
        ...
        if not fp16:
            ...
            #   获得预测结果
            #-------------------#
            _, regression, classification, anchors = model_train(images1,images2)
            # predict1 = F.sigmoid(classification).squeeze(1).cpu().detach().numpy()
            # print(predict1)
            #-------------------#
            #   计算损失
            #-------------------#
