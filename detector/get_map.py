...
if __name__ == "__main__":
    ...
    image_ids = open("").read().strip().split()
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        class_score = pd.DataFrame(columns=('id', 'y_pre', 'y_confidence'))
        for image_id in tqdm(image_ids):
            # 减影图
            image_path1  = os.path.join(""+image_id+".jpg")
            image1       = Image.open(image_path1)
            # 低能图
            image_path2 = os.path.join("" + image_id + ".jpg")
            image2 = Image.open(image_path2)
            if map_vis:
                image.save(os.path.join(map_out_path, "images-optional/" + image_id + ".jpg"))
            retinanet.get_map_txt(image_id, image1,image2, class_names, map_out_path)
            # class_score = class_score.append(pd.DataFrame(
            #     {'id': [image_id], 'y_pre': [cl], 'y_confidence': [score], }), ignore_index=True)
            # class_score.to_csv(" ")
    ...
    if map_mode == 0 or map_mode == 2:
        ...
        for image_id in tqdm(image_ids):
            with open(os.path.join(map_out_path, "ground-truth/"+image_id+".txt"), "w") as new_f:
                root = ET.parse(os.path.join(" "+image_id+".xml")).getroot()
